    ...
    internal class Encoder
    {
        ...
        private class LiteralEncoder
        {
            internal struct Encoder2
            {
                ...
                internal void Encode(RangeCoder.Encoder rangeEncoder, byte symbol)
                {
                    uint context = 1;
                    for (int i = 7; i >= 0; i--)
                    {
                ...
                internal void EncodeMatched(RangeCoder.Encoder rangeEncoder, byte matchByte, byte symbol)
                {
                    uint context = 1;
                    bool same = true;
                    for (int i = 7; i >= 0; i--)
        ...
        private class LenEncoder
        {
            ...
            internal void Init(UInt32 numPosStates)
            {
                _choice.Init();
                _choice2.Init();
            ...
            internal void Encode(RangeCoder.Encoder rangeEncoder, UInt32 symbol, UInt32 posState)
            {
            ...
            internal void SetPrices(UInt32 posState, UInt32 numSymbols, UInt32[] prices, UInt32 st)
            {
        ...
        private class LenPriceTableEncoder : LenEncoder
        {
            ...
            internal void SetTableSize(UInt32 tableSize)
            {
                _tableSize = tableSize;
            }

            internal UInt32 GetPrice(UInt32 symbol, UInt32 posState)
            {
            ...
            internal void UpdateTables(UInt32 numPosStates)
            {
                for (UInt32 posState = 0; posState < numPosStates; posState++)
                    UpdateTable(posState);
            }

            internal new void Encode(RangeCoder.Encoder rangeEncoder, UInt32 symbol, UInt32 posState)
            {
        ...
        {
            internal Base.State State;

            internal bool Prev1IsChar;
            internal bool Prev2;

            internal UInt32 PosPrev2;
            internal UInt32 BackPrev2;

            internal UInt32 Price;
            internal UInt32 PosPrev;
            internal UInt32 BackPrev;

            internal UInt32 Backs0;
            internal UInt32 Backs1;
            internal UInt32 Backs2;
            internal UInt32 Backs3;
        ...
        internal void Code(System.IO.Stream inStream, System.IO.Stream outStream,
            Int64 inSize, Int64 outSize)
        {
            _needReleaseMFStream = false;
            try
        ...
        internal void WriteCoderProperties(System.IO.Stream outStream)
        {
