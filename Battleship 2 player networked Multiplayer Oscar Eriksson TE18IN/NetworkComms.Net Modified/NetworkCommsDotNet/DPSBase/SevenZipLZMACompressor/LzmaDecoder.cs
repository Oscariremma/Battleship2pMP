...
namespace LZMA
{
    internal class Decoder
    {
        private class LenDecoder
        {
            ...
            internal uint Decode(RangeCoder.Decoder rangeDecoder, uint posState)
            {
        ...
        private class LiteralDecoder
        {
            private struct Decoder2
            {
                ...
                internal byte DecodeNormal(RangeCoder.Decoder rangeDecoder)
                {
                    uint symbol = 1;
                    do
                ...
                internal byte DecodeWithMatchByte(RangeCoder.Decoder rangeDecoder, byte matchByte)
                {
                    uint symbol = 1;
                    do
                    {
            ...
            internal byte DecodeNormal(RangeCoder.Decoder rangeDecoder, uint pos, byte prevByte)
            { return m_Coders[GetState(pos, prevByte)].DecodeNormal(rangeDecoder); }

            internal byte DecodeWithMatchByte(RangeCoder.Decoder rangeDecoder, uint pos, byte prevByte, byte matchByte)
